    ...
    public partial class frmQR : Form
    {
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            if (pictureBox1.Image != null)
            {
                ...
                {
                    timer1.Stop();
                    ucReturn.student_id = result.ToString();
        ...
        private void validate(string student_number)
        {
            sql.Query($"select * from transac_tb where student_id='{student_number}'");
            if (sql.HasException(true)) return;
            if(sql.DBDT.Rows.Count > 0)
            {
                MessageBox.Show(this, "Student found!", "Notice", MessageBoxButtons.OK, MessageBoxIcon.Information);
                get_id(student_number);
                scan_result = "Found";
                this.Close();
            }
            else
            {
                MessageBox.Show(this, "Student not found!", "Notice", MessageBoxButtons.OK, MessageBoxIcon.Information);
                scan_result = "Not Found";
                this.Close();
            }
        }
        ...
        {
            sql.Query($"select * from transac_tb where student_id ='{id}'");
            if (sql.HasException(true)) return;
            if(sql.DBDT.Rows.Count > 0)
            {
                foreach(DataRow dr in sql.DBDT.Rows)
                {
                    ucReturn.student_id = (dr["student_id"].ToString());
                }
            }
        }
